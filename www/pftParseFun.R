pftParse <- function(fileDir){
  ...
  for(i in 1:length(document)){
    ...
    report$id <- substr(doc[grep("Id", doc, ignore.case = TRUE)],gregexpr("Id", doc[grep("Id", doc, ignore.case = TRUE)])[[1]]+4, nchar(doc[grep("Id", doc, ignore.case = TRUE)]))
    ...
    scData[which(scData == "FEV1/FVC"):(which(scData == "FEV1/FVC")+4)],
    ...
    doc2 <- doc[grep("Lung Volumes", doc, ignore.case = TRUE)[1]:(grep("Diffusing", doc, ignore.case = TRUE)-1)] # extract spirometry data
    scData <- scan(text = doc2, what = "")
    switch <- 1
   if(length(which(scData == "TLC"))>1){ switch <- 2}
    ...
    doc2 <- doc[grep("Diffusing", doc):length(doc)] # extract spirometry data
    scData <- scan(text = doc2, what = "")
    ditch <- c(which(grepl("^\\(", scData)),
               which(grepl("\\)$", scData)),
               which(grepl("^-$", scData)))
    ...
    report$data <- rbind(report$data, df)
    
    report$data[,1] <- as.character(report$data[,1])
    report$data[,2] <- as.character(report$data[,2])
    report$data[,3] <- as.numeric(gsub("[^0-9\\.]", "", as.character(report$data[,3])))
    report$data[,4] <- as.numeric(gsub("[^0-9\\.]", "", as.character(report$data[,4])))
    report$data[,5] <- as.numeric(gsub("[^0-9\\.]", "", as.character(report$data[,5])))
